...
def gen_class(csr):
    ...
    call   = f"{name}_field.configure("
    indent = " " * (4 + len(call) + 1)   # выравнивание по .parent(this)
    lines.append(f"    {call} .parent(this),")
    lines.append(f"{indent}.size({length}),")
    lines.append(f"{indent}.lsb_pos(0),")
    lines.append(f"{indent}.access(\"RW\"),")
    lines.append(f"{indent}.volatile(0),")
    lines.append(f"{indent}.reset(64'h{reset:x}),")
    lines.append(f"{indent}.has_reset(1),")
    lines.append(f"{indent}.is_rand(1),")
    lines.append(f"{indent}.individually_accessible(1) );")
